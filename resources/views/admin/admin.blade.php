                ...
                <div class="modal-body" style="padding: 1.75rem;">
                    <form id="createAirportForm" method="POST" action="{{ route('admin.airport.store') }}">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="text-danger small mt-1" id="create-city-server-error">
                                @if($errors->hasBag('createAirport'))
                                    {{ $errors->getBag('createAirport')->first('city') }}
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="text-danger small mt-1" id="create-name-server-error">
                                @if($errors->hasBag('createAirport'))
                                    {{ $errors->getBag('createAirport')->first('name') }}
                                @endif
                            </div>
                        </div>
                ...
                <div class="modal-body" style="padding: 1.75rem;">
                    <form id="editAirportForm" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <div id="edit_city_error" class="text-danger small mt-1">
                                @if($errors->hasBag('editAirport'))
                                    {{ $errors->getBag('editAirport')->first('city') }}
                                @endif
                            </div>                    </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="text-danger small mt-1">
                                @if($errors->hasBag('editAirport'))
                                    {{ $errors->getBag('editAirport')->first('name') }}
                                @endif
                            </div>
                        </div>
    ...
    `;
        ...
        // ── UTENTI ──
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                @endif
                @if(old('name'))
                document.getElementById('airport_name').value = "{{ old('name') }}";
                @endif
            ...
            @if ($errors->hasBag('editAirport'))
            document.addEventListener('DOMContentLoaded', function() {
                document.getElementById('edit_country').value  = "{{ old('country') }}";
                document.getElementById('edit_city').value     = "{{ old('city') }}";
                document.getElementById('edit_airport_name').value = "{{ old('name') }}";
                document.getElementById('edit_latitude').value  = "{{ old('latitude') }}";
                document.getElementById('edit_longitude').value = "{{ old('longitude') }}";
            ...
            document.getElementById('createAirportModal').addEventListener('hidden.bs.modal', function () {
                ...
                document.querySelectorAll('#createAirportModal .text-danger').forEach(el => el.textContent = '');

                document.getElementById('create-name-server-error').textContent = '';
                document.getElementById('create-city-server-error').textContent = '';
            ...
            cityInput.addEventListener('input', () => {
                    ...
                    document.getElementById('create-city-server-error').textContent = ''; // ← aggiungi
                });
    ...
    <script>
        ...
        function initAutocomplete() {
            ...
            const editCountryAC = new google.maps.places.Autocomplete(editCountryInput, {
                types: ['(regions)'], fields: ['address_components'],
            });
            editCountryAC.addListener('place_changed', () => {
                const place = editCountryAC.getPlace();
                if (place?.address_components) {
                    const c = place.address_components.find(c => c.types.includes('country'));
                    if (c) editCountryInput.value = capitalizeWords(c.long_name.toLowerCase());
                }
            });

            const editCityAC = new google.maps.places.Autocomplete(editCityInput, {
                types: ['(cities)'], fields: ['address_components'],
            });
            editCityAC.addListener('place_changed', () => {
                const place = editCityAC.getPlace();
                if (place?.address_components) {
                    const c = place.address_components.find(c =>
                        c.types.includes('locality') ||
                        c.types.includes('administrative_area_level_2') ||
                    ...
                    if (c) editCityInput.value = capitalizeWords(c.long_name.toLowerCase());
                }
            });
        }
        ...
        document.getElementById('airport_name').addEventListener('input', () => {
            document.getElementById('create-name-server-error').textContent = ''; // ← aggiungi
            checkCreateAirportForm();
