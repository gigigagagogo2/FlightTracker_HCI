            ...
            @auth
                @if(auth()->user()->is_admin)
                    <li class="nav-item me-3">
                        <a class="nav-link d-flex align-items-center" href="{{ route('admin.dashboard') }}" title="Area personale">
                            <img src="/images/admin_profile.png" alt="Profilo" class="profile-pic" style="width: 24px; height: 24px;">
                        </a>
                    </li>
                @else
                    <li class="nav-item me-3">
                        <a class="nav-link d-flex align-items-center" href="{{ route('user.personal.map') }}" title="Mappa personale">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" class="map-icon" viewBox="0 0 16 16" style="filter: drop-shadow(2px 2px 4px rgba(0,0,0,0.3)); transition: all 0.3s ease; margin-right: 6px;">
                                <defs>
                    ...
                    <li class="nav-item me-3">
                        <a class="nav-link d-flex align-items-center" href="{{ route('user.profile') }}" title="Area personale">
                            @if(auth()->user()->profile_picture_path)
                                <img src="{{ route('profile.picture', ['filename' => auth()->user()->profile_picture_path])  }}" alt="Profilo" class="profile-pic" style="width: 24px; height: 24px;">
                            @else
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="profile-icon" viewBox="0 0 16 16" style="margin-right: 6px;">
                ...
                <li class="nav-item">
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="nav-link btn btn-link text-decoration-none" title="Logout" style="padding: 0; border: none; background: none;">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-box-arrow-right" viewBox="0 0 16 16">
                                <path fill-rule="evenodd" d="M10 15a1 1 0 0 0 1-1v-4h-1v4H3V2h7v4h1V2a1 1 0 0 0-1-1H3a1 1 0 0 0-1 1v12a1 1 0 0 0 1 1h7z"/>
