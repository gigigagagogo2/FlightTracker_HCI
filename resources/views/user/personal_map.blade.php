    ...
    <div id="flightInfoCard" class="card position-absolute bottom-0 start-50 translate-middle-x mb-4 shadow"
        ...
        <div class="card-body position-relative">
            ...
            <div class="progress position-relative" style="height: 30px;">
                <div id="flightProgressLabel" class="position-absolute w-100 text-center fw-bold"
                     style="line-height: 30px; z-index: 2;">
                    0%
                </div>
                <div id="flightProgress" class="progress-bar bg-warning text-dark" role="progressbar"
                     style="width: 0; z-index: 1;"></div>
            </div>
        </div>
    </div>
</main>
        ...
        try {
            ...
            const flightPromises = flights.map(async (flight) => {
                ...
                overlay.addListener('click', () => {
                    ...
                    const progress = Math.round(flight.progress * 100);
                    if (typeof flight.progress === 'number' && !isNaN(flight.progress))
                    {
                        document.getElementById('flightProgress').style.width = `${progress}%`;
                        document.getElementById('flightProgressLabel').innerText = `${progress}%`;
                    }
                });
        ...
        try {
            ...
            for (const flightId in data) {
                ...
                if (parseInt(flightId) === currentFlightId) {
                    ...
                    const progress = Math.round(flightData.progress * 100);
                    if (typeof flightData.progress === 'number' && !isNaN(flightData.progress)) {
                        document.getElementById('flightProgress').style.width = `${progress}%`;
                        document.getElementById('flightProgressLabel').innerText = `${progress}%`;
                    }
                }
